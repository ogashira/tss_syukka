...
- 999998の場合、受注入力時に品名の最後のカッコ内に重量と単位を記入すること。最後のカッコ内の数値を読み取って、総重量を計算するため。単位はKGまたはG（大文字小文字どちらでもOK)
    - 例）UV-TEST3アンダー(15kg)
        - 以下はNG例
            - **UV-TEST3アンダー(15kg)試作3(サンプル)** &nbsp;   <- 重量が最後のカッコではないため
            - **UV-TEST3アンダー(15)試作3** &nbsp;  <- カッコ内に単位がないため
            - **UV-TEST3(改)アンダー(15&nbsp;kg)** &nbsp;  <- 重量とkgの間にスペースがあるため
        - 以下はOK例
            - **UV-TEST3(改)アンダー(15kg)試作3**&nbsp; <-  最後のカッコに重量と単位がある
            - **UV-TEST3(改良3)アンダー(試作品)(500g)サンプル**&nbsp; <- 最後のカッコに重量と単位がある
---
### アウトプット連絡表
